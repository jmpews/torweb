	    ...
	    switch (action.type) {
	        ...
	        case _ActionTypes.RECEIVE_A_MESSAGE:
	            ...
	            if (state.current_user.id != message.id) {
	                ...
	                return x.setIn(['cha', 'recent_message_list', 'msg'], z);
	            }
	        case _ActionTypes.RECENT_USER_LIST:
	            return (0, _immutable.fromJS)(state).setIn(['chat', 'recent_user_list'], action.payload).toJS();

	        default:
	            return (0, _immutable.fromJS)(state).setIn(['chat', 'recent_message_list'], update_recnet_message_list(state.recent_message_list, action)).toJS();
	    }
	};
